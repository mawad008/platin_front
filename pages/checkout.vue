    ...
    <div class="container checkout-page" style="">
      ...
      10.8415 3.18625 10.3519 3.18625 9.74793V9.73715C3.18625 9.13317 2.69663 8.64355 2.09266 8.64355Z"
              ...
              <div class="col-12 col-xl-4 col-lg-4">
                <checkProduct
                  :arrData="arrData"
                  :myFunction="handlecheckoutFunc1"
                  :text="text2"
                  :pending="pending"
            ...
            <div class="card-popup-container">
              <div class="main">
                <div>
                  ...
                  <p class="w-100" v-if="!messagePopup" >{{checkCityProducts ? $t("shippText") :  $t("shippText2") }}</p>
                  <p class="w-100" v-else > {{ messagePopup }} </p>
                </div>
                <div class="boxes">
                  <div
...
const { initMap , clickedLat, clickedLng , marker } = useMap();
...
const { returnV , returnV2 } = useValidation();
const { checkoutFunc , checkoutFunc1 , pending , errors , dialog , messagePopup , checkCityProducts , productsPopup , destination , errors1 , unique_vendorsVar , priceShipping } = useCheckout();
let v$ = returnV(userdata1);

const handlecheckoutFunc = ()=>{
  checkoutFunc(v$.value , personalorGift.value , userdata1.value , gift_owner_name.value , gift_owner_phone.value , gift_text.value)
}
...
let v2$ = returnV2(userdata2);

const handlecheckoutFunc1 = ()=>{
  checkoutFunc1(v2$.value , userdata2.value , clickedLat.value , clickedLng.value , userdata1.value , newArr.value , personalorGift.value , gift_owner_name.value , gift_owner_phone.value , gift_text.value , chooseCity.value)
}
  ...
  try {
    let result = await axios.post(
      `${getUrl()}/orders-checkout`,
      {
        ...
        // total:store.state.totalNum + (priceShipping ? priceShipping : 0) + (store.state.totalNum * 0.15),
...
const paymentFunc = async () => {
  ...
  try {
    ...
    if(destination.value){
      formBody.append("destination", destination.value);
    }
    formBody.append("price", store.state.totalNum + (priceShipping.value ? priceShipping.value : 0) + (store.state.totalNum * 0.15));
    let result = await axios.post(`${getUrl()}/order-pay`, {
      first_name: userdata1.value.first_name,
      last_name: userdata1.value.last_name,
      phone: userdata1.value.phone,
      email: userdata1.value.email,
      price: store.state.totalNum + (priceShipping.value ? priceShipping.value : 0) + (store.state.totalNum * 0.15),
...
watch(
  ...
  initMap();
    ...
    if (user1) {
      ...
      userdata1.value.last_name = user1?.userdata1?.last_name ? user1?.userdata1?.last_name : "" || loginUser.last_name ? loginUser.last_name : "";
      userdata1.value.phone = user1?.userdata1?.phone ? user1?.userdata1?.phone : "" || loginUser.phone ? loginUser.phone : "";
      userdata1.value.email = user1?.userdata1?.email ? user1?.userdata1?.email : "" || loginUser.email ? loginUser.email : "";
      personalorGift.value = user1.personalorGift ? user1.personalorGift : 2;
